...
class ConfigTests(unittest.TestCase):
    ...
    def test_load_portfolio_mappings_exposes_prediction_weights(self) -> None:
        ...
        self.assertTrue(bool(mappings["PREDICTION_WEIGHTS"]["calibration"]["family_enabled"]))
        self.assertTrue(bool(mappings["PREDICTION_WEIGHTS"]["calibration"]["global_shrink_enabled"]))
        self.assertEqual(mappings["ARGENTINA_EQUITY_MAP"]["PAMP"]["asset_subfamily"], "stock_argentina")
        self.assertEqual(mappings["FINVIZ_MAP"]["O"], "O")
        self.assertEqual(mappings["BLOCK_MAP"]["O"], "Dividendos")
        self.assertEqual(mappings["INSTRUMENT_PROFILE_MAP"]["O"]["asset_subfamily"], "stock_defensive_dividend")
